...
const [routes,setRoutes]=useState<RouteRow[]>([]);const [expenseProjects,setExpenseProjects]=useState<ExpenseProject[]>([]);const [donors,setDonors]=useState<Donor[]>([]);const [members,setMembers]=useState<Member[]>([]);const [receipts,setReceipts]=useState<Receipt[]>([]);const [expenses,setExpenses]=useState<Expense[]>([]);const [quotations,setQuotations]=useState<Quotation[]>([]);const [users,setUsers]=useState<AppUser[]>([]);const [receiptAudits,setReceiptAudits]=useState<ReceiptAudit[]>([]);const [driveConnected,setDriveConnected]=useState(false);const [driveBusy,setDriveBusy]=useState(false);const [bulkDriveUpload,setBulkDriveUpload]=useState({running:false,total:0,done:0,uploaded:0,skipped:0,failed:0,current:''})
 ...
 async function loadAll(userId?:string){
  ...
  try{
   ...
   const [orgR,routeR,donorR,peopleR,paymentR,receiptR,userR,auditR,expenseR,quotationR,memberFeeR,expenseProjectR]=await Promise.all([
    ...
    supabase.from('expense_projects').select('*').eq('organization_id',p.organization_id).eq('active',true).order('name')
   ])
...
*– राजस्थान युवक मंडळ, संगमनेर*`)
 ...
 const membershipFeePaidCount=useMemo(()=>members.filter(m=>m.annualFeePaid>0&&(m.annualFee<=0||m.annualFeePaid>=m.annualFee)).length,[members])
 ...
 {active==='expenses'&&canManageExpenses&&<ExpenseManager supabase={supabase} orgId={orgId} profile={profile!} members={members} projects={expenseProjects} expenses={expenses} quotations={quotations} reload={()=>loadAll()} />}
...
function PendingPayments({receipts,onSettle,onReminder}:{receipts:Receipt[],onSettle:(r:Receipt)=>void,onReminder:(r:Receipt)=>void}){
 ...
 const allRows=receipts.filter(r=>r.paymentStatus==='receipt_pending')
 const rows=allRows.filter(r=>searchMatches(q,r.donorName,r.mobile,r.receiptNo,r.areaName,r.amount))
 async function downloadExcel(){
  const XLSX=await import('xlsx')
  if(!allRows.length)return alert('No Receipt Given - Payment Pending donors found.')
  const data=allRows.slice().sort((a,b)=>String(a.areaName||'').localeCompare(String(b.areaName||''),'en',{numeric:true,sensitivity:'base'})||a.donorName.localeCompare(b.donorName,'en',{sensitivity:'base'})).map((r,i)=>({
   'SrNo':i+1,'Receipt Number':r.receiptNo,'Donor Name':r.donorName,'Mobile Number':r.mobile||'','Receipt Date':r.date,'Area / Route':r.areaName||'','Amount':r.amount,'Payment Status':'Payment Pending'
  }))
  const ws=XLSX.utils.json_to_sheet(data,{header:['SrNo','Receipt Number','Donor Name','Mobile Number','Receipt Date','Area / Route','Amount','Payment Status']})
  ws['!cols']=[{wch:8},{wch:16},{wch:32},{wch:18},{wch:16},{wch:24},{wch:14},{wch:20}]
  const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,'Payment Pending')
  XLSX.writeFile(wb,`RYM_VARGANI-Receipt-Given-Payment-Pending-${new Date().toISOString().slice(0,10)}.xlsx`)
 }
 return <section className="card donorCard">
  <div className="tableHead"><div><b>Receipt Given - Payment Pending</b><small>{allRows.length} pending receipts · {money(allRows.reduce((sum,r)=>sum+r.amount,0))}{q.trim()?` · ${rows.length} matching`:''}</small></div><div className="actions"><input className="search" placeholder="Search donor, mobile, receipt no. or area..." value={q} onChange={e=>setQ(e.target.value)}/><button className="ghost" onClick={downloadExcel}>↓ Download Excel</button></div></div>
  <div className="tableWrap"><table><thead><tr><th>Receipt No.</th><th>Donor</th><th>Date</th><th>Area</th><th>Amount</th><th></th></tr></thead>
   <tbody>{rows.map(r=><tr key={r.id}><td><b>{r.receiptNo}</b></td><td>{r.donorName}<small>{r.mobile}</small></td><td>{r.date}</td><td>{r.areaName}</td><td><b>{money(r.amount)}</b></td><td><div className="rowActions pendingReceiptActions"><button className="smallBtn primary" onClick={()=>onSettle(r)}>Mark Payment Received</button><button className="smallBtn reminderBtn" onClick={()=>onReminder(r)}>Send Reminder</button></div></td></tr>)}</tbody>
  </table>{!allRows.length&&<div className="empty">No pending receipt payments.</div>}{!!allRows.length&&!rows.length&&<div className="empty">No matching pending receipt found.</div>}</div>
 </section>
}
...
function ExpenseManager({supabase,orgId,profile,members,projects,expenses,quotations,reload}:{supabase:any;orgId:string;profile:AppUser;members:Member[];projects:ExpenseProject[];expenses:Expense[];quotations:Quotation[];reload:()=>void}){
 const [tab,setTab]=useState<'expenses'|'memberPaid'|'quotations'>('expenses'),[showExpense,setShowExpense]=useState(false),[showQuotation,setShowQuotation]=useState(false),[showProject,setShowProject]=useState(false),[projectName,setProjectName]=useState(''),[projectDescription,setProjectDescription]=useState(''),[projectFilter,setProjectFilter]=useState('All Projects'),[editing,setEditing]=useState<Expense|null>(null),[paying,setPaying]=useState<Expense|null>(null)
 ...
 const blank=()=>({projectId:'',expenseType:'with_bill',head:'Decoration',billName:'',invoiceNumber:'',vendorName:'',expenseDate:new Date().toISOString().slice(0,10),amount:'',gstAmount:'',description:'',payerType:'mandal',memberId:'',paymentMode:'Cash',paymentDate:new Date().toISOString().slice(0,10),chequeNumber:'',bankName:'',chequeDate:''})
 ...
 const visibleExpenses=projectFilter==='All Projects'?expenses:expenses.filter(x=>x.projectId===projectFilter)
 ...
 function openEdit(x:Expense){setEditing(x);setExpenseFile(null);setEf({projectId:x.projectId,expenseType:x.expenseType,head:x.head,billName:x.billName,invoiceNumber:x.invoiceNumber,vendorName:x.vendorName,expenseDate:x.expenseDate,amount:String(x.amount),gstAmount:String(x.gstAmount||''),description:x.description,payerType:x.payerType,memberId:x.memberId,paymentMode:x.paymentMode||'Cash',paymentDate:x.paymentDate||new Date().toISOString().slice(0,10),chequeNumber:x.chequeNumber,bankName:x.bankName,chequeDate:x.chequeDate});setShowExpense(true)}
 async function saveExpense(asPending:boolean){if(!ef.amount||Number(ef.amount)<=0)return alert('Enter expense amount.');if(ef.expenseType==='with_bill'&&(!ef.billName.trim()||!ef.vendorName.trim()))return alert('Bill Name and Vendor Name are required.');if(ef.payerType==='member'&&!ef.memberId)return alert('Select member.');if(!asPending&&ef.payerType==='mandal'&&ef.paymentMode==='Cheque'&&!ef.chequeNumber.trim())return alert('Cheque Number is required.')
  try{const url=expenseFile?await upload(expenseFile,'expenses'):(editing?.attachmentUrl||''),m=members.find(x=>x.id===ef.memberId),memberPaid=ef.payerType==='member';const project=projects.find(p=>p.id===ef.projectId);const row:any={organization_id:orgId,project_id:ef.projectId||null,project_name:project?.name||null,expense_type:ef.expenseType,expense_head:ef.head,bill_name:ef.billName||null,invoice_number:ef.invoiceNumber||null,vendor_name:ef.vendorName||null,expense_date:ef.expenseDate,amount:Number(ef.amount),gst_amount:Number(ef.gstAmount||0),description:ef.description||null,payer_type:ef.payerType,member_id:memberPaid?ef.memberId:null,member_name:memberPaid?(m?.name||null):null,attachment_url:url||null}
 ...
 async function saveProject(){
  ...
  const {error}=await supabase.from('expense_projects').insert({organization_id:orgId,name:projectName.trim(),description:projectDescription.trim()||null,active:true,created_by:profile.userId,created_by_name:profile.name})
  if(error)return alert(error.message)
  setProjectName('');setProjectDescription('');setShowProject(false);reload()
 }
 ...
 return <div className="expenseModule"><section className="stats expenseStats"><Stat label="Mandal Expenses" value={money(total)} icon="₹"/><Stat label="Payment Pending" value={money(pending)} icon="⌛" warn/><Stat label="Member Return Pending" value={money(memberPendingTotal)} icon="♙"/><Stat label="Quotations" value={String(quotations.length)} icon="▤"/></section><section className="card">
  ...
  {tab==='expenses'?<><div className="tableHead"><div><b>Mandal Expenses / Inward Bills</b><small>Mandal Expenses above is the total of all projects</small></div><div className="actions"><select value={projectFilter} onChange={e=>setProjectFilter(e.target.value)}><option>All Projects</option>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><button className="ghost" onClick={()=>setShowProject(true)}>+ Add Project</button><button className="primary" onClick={openNew}>+ Add Expense</button></div></div><div className="tableWrap"><table><thead><tr><th>Date</th><th>Project</th><th>Head</th><th>Bill / Vendor</th><th>Amount</th><th>Who Paid</th><th>Status</th><th>Bill</th><th>Action</th></tr></thead><tbody>{visibleExpenses.map(x=><tr key={x.id}><td>{x.expenseDate}</td><td><b>{x.projectName||'General'}</b></td><td><b>{x.head}</b><small>{x.expenseType==='with_bill'?'With Bill':'Without Bill'}</small></td><td>{x.billName||'—'}<small>{x.vendorName}</small></td><td><b>{money(x.amount)}</b></td><td>{x.payerType==='member'?x.memberName:'Self / Mandal'}</td><td>{x.payerType==='member'?(x.reimbursementStatus==='returned'?'Returned / Mandal Expense':'Return Pending'):(x.paymentStatus==='paid'?'Paid':'Payment Pending')}</td><td>{x.attachmentUrl?<a className="smallBtn" href={x.attachmentUrl} target="_blank" rel="noreferrer">View</a>:'—'}</td><td><div className="rowActions"><button className="smallBtn" onClick={()=>openEdit(x)}>✎ Edit</button><button className="smallBtn dangerBtn" onClick={()=>del(x)}>🗑 Delete</button>{x.payerType==='mandal'&&x.paymentStatus==='pending'&&<button className="smallBtn" onClick={()=>setPaying(x)}>Mark Paid</button>}</div></td></tr>)}</tbody></table></div></>
 ...
 {showProject&&<div className="overlay"><div className="modal"><div className="modalHead"><div><small>Expense Management</small><h3>Add Project</h3></div><button onClick={()=>setShowProject(false)}>×</button></div><div className="formGrid"><Field label="Project Name *"><input value={projectName} onChange={e=>setProjectName(e.target.value)}/></Field><Field label="Description"><textarea value={projectDescription} onChange={e=>setProjectDescription(e.target.value)}/></Field></div><div className="modalFoot"><button className="ghost" onClick={()=>setShowProject(false)}>Cancel</button><button className="primary" onClick={saveProject}>Save Project</button></div></div></div>}
 {showExpense&&<div className="overlay"><div className="modal wideModal"><div className="modalHead"><div><small>Expense Management</small><h3>{editing?'Edit Expense':'Add Expense'}</h3></div><button onClick={()=>setShowExpense(false)}>×</button></div><div className="formGrid">
  <Field label="Project"><select value={ef.projectId} onChange={e=>setEf({...ef,projectId:e.target.value})}><option value="">General / No Project</option>{projects.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></Field><Field label="Expense Type *"><select value={ef.expenseType} onChange={e=>setEf({...ef,expenseType:e.target.value})}><option value="with_bill">With Bill</option><option value="without_bill">Without Bill</option></select></Field><Field label="Expense Head *"><select value={ef.head} onChange={e=>setEf({...ef,head:e.target.value})}>{heads.map(h=><option key={h}>{h}</option>)}</select></Field>
...
function MemberManager({supabase,orgId,members,canManage,isAdmin,onAdd,onBulk,reload}:{supabase:any;orgId:string;members:Member[];canManage:boolean;isAdmin:boolean;onAdd:()=>void;onBulk:()=>void;reload:()=>void}){
 async function downloadMembersExcel(){
  ...
  const rows=members.slice().sort((a,b)=>a.name.localeCompare(b.name,'en',{sensitivity:'base'})).map((m,i)=>{
   ...
   return {'SrNo':i+1,'Member Name':m.name,'Mobile Number':m.mobile||'','Birthdate':m.birthDate||'','Annual Fees':m.annualFee||0,'Payment Status':m.annualFeePaid>0&&(m.annualFee<=0||pending<=0)?'Paid':'Pending'}
  })
 ...
 const [collecting,setCollecting]=useState<Member|null>(null),[editingMember,setEditingMember]=useState<Member|null>(null),[memberSearch,setMemberSearch]=useState(''),[memberStatusFilter,setMemberStatusFilter]=useState<'All'|'Paid'|'Pending'>('All'),[mf,setMf]=useState({name:'',mobile:'',birthDate:'',annualFee:''}),[cf,setCf]=useState({amount:'',mode:'Cash',date:new Date().toISOString().slice(0,10),chequeNumber:'',bankName:''})
 const normalizedMemberSearch=memberSearch.trim().toLowerCase().replace(/\s+/g,' ')
 const memberIsPaid=(m:Member)=>m.annualFeePaid>0&&(m.annualFee<=0||m.annualFeePaid>=m.annualFee)
 const filteredMembers=members.filter(m=>{
  const matchesSearch=!normalizedMemberSearch||`${m.name} ${m.mobile} ${m.birthDate}`.toLowerCase().replace(/\s+/g,' ').includes(normalizedMemberSearch)
  const paid=memberIsPaid(m)
  const matchesStatus=memberStatusFilter==='All'||(memberStatusFilter==='Paid'?paid:!paid)
