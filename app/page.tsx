...
export default function Home(){
 ...
 const isAdmin=profile&&['Super Admin','Admin'].includes(profile.role);const canManageExpenses=!!profile&&['Super Admin','Admin','Treasurer'].includes(profile.role);const isSuperAdmin=profile?.role==='Super Admin';const canDeleteDonor=!!profile&&['Super Admin','Admin','Treasurer'].includes(profile.role);const canDownloadReports=!!profile&&['Super Admin','Admin','Treasurer'].includes(profile.role)
...
function MemberManager({supabase,orgId,members,canManage,isAdmin,onAdd,onBulk,reload}:{supabase:any;orgId:string;members:Member[];canManage:boolean;isAdmin:boolean;onAdd:()=>void;onBulk:()=>void;reload:()=>void}){
 ...
 const [collecting,setCollecting]=useState<Member|null>(null),[cf,setCf]=useState({amount:'',mode:'Cash',date:new Date().toISOString().slice(0,10),chequeNumber:'',bankName:''})
