...
export default function Home(){
 ...
 async function settlePendingReceipt(receipt:Receipt){
  if(!supabase)return
  const mode=window.prompt('Enter payment method: Cash / UPI / Bank / Cheque','Cash')?.trim()
  if(!mode)return
  if(!['cash','upi','bank','cheque'].includes(mode.toLowerCase()))return alert('Please enter Cash, UPI, Bank or Cheque.')
  let chequeNo:string|null=null,bankName:string|null=null
  if(mode.toLowerCase()==='cheque'){chequeNo=window.prompt('Cheque Number')?.trim()||null;if(!chequeNo)return alert('Cheque Number is required.');bankName=window.prompt('Bank Name')?.trim()||null}
  const date=window.prompt('Payment received date (YYYY-MM-DD)',new Date().toISOString().slice(0,10))?.trim()
  if(!date)return
  const {error}=await supabase.rpc('settle_pending_payment',{p_payment_id:receipt.paymentId,p_mode:mode.toLowerCase(),p_cheque_no:chequeNo,p_bank_name:bankName,p_payment_date:date})
 ...
 async function saveReceiptEdit(){
  ...
  if(error)return alert(error.message)
  setModal(null);setEditReceipt(null);await loadAll();alert('Receipt updated. The change has been recorded in the Admin audit log.')
 }
 ...
 const shown=donors.filter(d=>(active!=='route'||((!selectedRoute||d.routeId===selectedRoute)&&d.status!=='Collected'))&&(active!=='pending'||(d.status!=='Collected'&&(!pendingRoute||d.routeId===pendingRoute)))&&`${d.name} ${d.contactPerson} ${d.mobile} ${d.route} ${d.reference}`.toLowerCase().includes(q.toLowerCase())).sort((a,b)=>active==='pending'?((a.route||'').localeCompare(b.route||'',undefined,{sensitivity:'base'})||(a.name||'').localeCompare(b.name||'',undefined,{sensitivity:'base'})):0)
 ...
 {active==='reports'&&<Reports donors={donors} receipts={receipts}/>} {active==='paymentPending'&&<PendingPayments receipts={receipts} onSettle={settlePendingReceipt}/>} {active==='daywise'&&<DaywiseReport receipts={receipts} date={dayReportDate} setDate={setDayReportDate}/>} 
...
function DaywiseReport({receipts,date,setDate}:{receipts:Receipt[],date:string,setDate:(v:string)=>void}){
 ...
 const total=(mode:string)=>paidRows.filter(r=>r.mode.toLowerCase()===mode.toLowerCase()).reduce((s,r)=>s+r.amount,0)
 const cash=total('Cash'),upi=total('UPI'),cheque=total('Cheque'),pending=pendingRows.reduce((s,r)=>s+r.amount,0),dayTotal=cash+upi+cheque
 const rows=[...paidRows,...pendingRows]
 return <><section className="card dayFilter"><div><b>Day-wise Collection Report</b><small>Paid amounts use Payment Received Date; pending receipts use Receipt Date</small></div><input type="date" value={date} onChange={e=>setDate(e.target.value)}/></section><section className="stats dayStats"><Stat label="Cash Collection" value={money(cash)} icon="₹"/><Stat label="UPI Collection" value={money(upi)} icon="↗" accent/><Stat label="Cheque Collection" value={money(cheque)} icon="▤"/><Stat label="Receipt Issued - Payment Pending" value={money(pending)} icon="⌛" warn/><Stat label="Total Day's Collection" value={money(dayTotal)} icon="Σ" accent/></section><section className="card"><b>Activity on {display}</b><div className="tableWrap"><table><thead><tr><th>Receipt</th><th>Donor</th><th>Receipt Date</th><th>Payment Received Date</th><th>Mode</th><th>Status</th><th>Amount</th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td>{r.receiptNo}</td><td>{r.donorName}</td><td>{r.date}</td><td>{r.paymentReceivedDate||'—'}</td><td>{r.mode}</td><td>{r.paymentStatus==='receipt_pending'?'Payment Pending':'Paid'}</td><td>{money(r.amount)}</td></tr>)}</tbody></table>{!rows.length&&<div className="empty">No collection activity on this date.</div>}</div></section></>}

function Reports({donors,receipts}:{donors:Donor[],receipts:Receipt[]}){
 const total=receipts.filter(r=>r.paymentStatus==='paid').reduce((s,r)=>s+r.amount,0),pending=donors.reduce((s,d)=>s+Math.max(0,d.expected-d.received),0)
 const modes=['Cash','UPI','Bank','Cheque','Receipt Given - Payment Pending'].map(mode=>({mode,amount:receipts.filter(r=>r.mode.toLowerCase()===mode.toLowerCase()).reduce((s,r)=>s+r.amount,0)}))
