...
export default function Home(){
 ...
 async function sendDashboardReceipt(receipt:Receipt){
  const mobile=donorWhatsappNumber(receipt.mobile)
 ...
 const shown=donors.filter(d=>(active!=='route'||((!selectedRoute||d.routeId===selectedRoute)&&d.status!=='Collected'&&!pendingReceiptByDonor.has(d.id)))&&(active!=='pending'||(d.status!=='Collected'&&(!pendingRoute||d.routeId===pendingRoute)))&&`${d.name} ${d.contactPerson} ${d.mobile} ${d.route} ${d.reference}`.toLowerCase().includes(q.toLowerCase())).sort((a,b)=>active==='pending'?((a.route||'').localeCompare(b.route||'',undefined,{sensitivity:'base'})||(a.name||'').localeCompare(b.name||'',undefined,{sensitivity:'base'})):0)
